...
\texttt{GROUP BY uuid}.  The special rule \syntax{<termValue := <value>
`@time'} in Figure~\ref{fig:temporalgrammar} returns the valid timestamp of the
...
\todo{we probably want to demonstrate how to combine the selct + where clauses to cover each of the 7 segments}
